...
- 答题卷（kind=1）：题库（建库、库内题目增删改）、从题库勾选题并导入答题卷（顺序/随机排列，随机时每位答题者顺序不同）、题目分值（1-1000）与自动判分、倒计时（到时自动交卷）、答案设定与「提交后展示答案」开关（分页模式下逐题提交：每题「提交本题」立即判分展示答案并锁定不可修改）、列表/分页两种展示方式（互斥，均左侧常显答题卡并与题目同行起始：题号 5 列排布、标记已答/未答，逐题提交时题号颜色跟随对错，分页点击题号跳题、列表点击题号滚动定位）、答题前可配置的个人信息收集（字段自定义）、答题后可配置的排行榜（得分降序 + 耗时升序）、统计含平均得分与明细得分/个人信息、CSV 导出含得分列
...
- AI 能力（Agent）：管理员配置任意 OpenAI 兼容服务后，用户可「AI 创建问卷」「AI 创建答题」「AI 生成题库题目」「自然语言对话改题」「单题 AI 优化」「开放题 AI 摘要」；AI 修改以提案呈现，用户确认后仍走手动保存，AI 无直接写库路径；每用户每日配额可调
