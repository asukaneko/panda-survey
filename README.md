...
- 答题卷（kind=1）：题库（建库、库内题目增删改）、从题库勾选题并导入答题卷（顺序/随机排列，随机时每位答题者顺序不同）、题目分值（1-1000）与自动判分、倒计时（到时自动交卷）、答案设定与「提交后展示答案」开关（分页模式下逐题提交：每题「提交本题」立即判分展示答案并锁定不可修改）、列表/分页两种展示方式（互斥，分页为逐题作答且答完最后一题出现答题卡，题号标记已提交/未提交并可点击跳转）、答题前可配置的个人信息收集（字段自定义）、答题后可配置的排行榜（得分降序 + 耗时升序）、统计含平均得分与明细得分/个人信息、CSV 导出含得分列
